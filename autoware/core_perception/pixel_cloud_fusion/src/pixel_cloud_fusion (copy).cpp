...
ROSPixelCloudFusionApp::TransformPoint(const velodyne_pointcloud::PointXYZIR &in_point, const tf::StampedTransform &in_transform)
{
...
void ROSPixelCloudFusionApp::CloudCallback(const sensor_msgs::PointCloud2::ConstPtr &in_cloud_msg)
{
	...
	std::unordered_map<cv::Point, velodyne_pointcloud::PointXYZIR> projection_map;
	...
	for (size_t i = 0; i < in_cloud->points.size(); i++)
	{
		...
		{
			projection_map.insert(std::pair<cv::Point, velodyne_pointcloud::PointXYZIR>(cv::Point(u, v), in_cloud->points[i]));
		}
	}
	...
	pcl::toROSMsg(*out_cloud_intensity, cloud_msg2);
	cloud_msg2.header = in_cloud_msg->header;
	publisher_fused_cloud_intensity.publish(cloud_msg2);
	////////////////////////////////////////////////
}
...
void ROSPixelCloudFusionApp::InitializeROSIo(ros::NodeHandle &in_private_handle)
{
	...
	publisher_fused_cloud_intensity = node_handle_.advertise<sensor_msgs::PointCloud2>(fused_topic_str+"_intensity", 1);
