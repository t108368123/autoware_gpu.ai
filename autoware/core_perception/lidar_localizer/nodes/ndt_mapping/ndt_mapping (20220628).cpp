...
static bool _use_imu_gnss_init_pose = false;
...
bool gnss_init_is_set = false;
bool imu_init_is_set = false;

double imu_roll_temp, imu_pitch_temp, imu_yaw_temp;
double gnss_x_temp, gnss_y_temp;
...
static void output_callback(const autoware_config_msgs::ConfigNDTMappingOutput::ConstPtr& input)
{
  ...
  {    
    ...
    cloud_intensity=*map_ptr;
    for (pcl::PointCloud<pcl::PointXYZI>::const_iterator item = cloud_intensity.begin(); item != cloud_intensity.end(); item++)
    {
      p2.x = (double)item->x;
      p2.y = (double)item->y;
      p2.z = (double)item->z;
      p2.rgb = (float)item->intensity;

      cloud_rgb.push_back(p2);
    }
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr map_rgb_ptr(new pcl::PointCloud<pcl::PointXYZRGB>(cloud_rgb));
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr map_rgb_filtered(new pcl::PointCloud<pcl::PointXYZRGB>());
    sensor_msgs::PointCloud2::Ptr map_msg_ptr(new sensor_msgs::PointCloud2);
    map_rgb_ptr->header.frame_id = "map";
    map_rgb_filtered->header.frame_id = "map";
    ///////////////////////////////////
    // Apply voxelgrid filter
    if (filter_res == 0.0)
    {
      std::cout << "Original: " << map_rgb_ptr->points.size() << " points." << std::endl;
    ...
    else
    {
      ...
      voxel_grid_filter.setInputCloud(map_rgb_ptr);
      voxel_grid_filter.filter(*map_rgb_filtered);
      std::cout << "Original: " << map_rgb_ptr->points.size() << " points." << std::endl;
      std::cout << "Filtered: " << map_rgb_filtered->points.size() << " points." << std::endl;
      pcl::toROSMsg(*map_rgb_filtered, *map_msg_ptr);
    }
    ...
    if (filter_res == 0.0)
    {
      pcl::io::savePCDFileASCII(filename, *map_rgb_ptr);
      std::cout << "rgb_Saved " << map_rgb_ptr->points.size() << " data points to " << filename << "." << std::endl;
    }
    else
    {
      pcl::io::savePCDFileASCII(filename, *map_rgb_filtered);
      std::cout << "rgb_Saved " << map_rgb_filtered->points.size() << " data points to " << filename << "." << std::endl;
    }
  }
  else
  {
...
static void imu_callback(const sensor_msgs::Imu::Ptr& input)
{
  ...
  imu_roll_temp = imu_roll;
  imu_pitch_temp = imu_pitch;
  imu_yaw_temp = imu_yaw;
  if(imu_init_is_set == false && _use_imu_gnss_init_pose == true)
  {
    imu_init_is_set = true;
    std::cout << "imu_init_is_set!" << std::endl;
  }
...
static void points_callback(const sensor_msgs::PointCloud2::ConstPtr& input)
{
  ...
  if(_use_imu_gnss_init_pose == true && set_flag==false)
  {
    if(gnss_init_is_set == false || imu_init_is_set == false)
  	  return;
    if(gnss_init_is_set == true && imu_init_is_set == true)
    {
      Eigen::Translation3f tl_btol(gnss_x_temp, gnss_y_temp, _tf_z);                 // tl: translation
      Eigen::AngleAxisf rot_x_btol(imu_roll_temp, Eigen::Vector3f::UnitX());  // rot: rotation
      Eigen::AngleAxisf rot_y_btol(imu_pitch_temp, Eigen::Vector3f::UnitY());
      Eigen::AngleAxisf rot_z_btol(imu_yaw_temp, Eigen::Vector3f::UnitZ());
  ...
  pcl::PointCloud<pcl::PointXYZRGB> tmp_rgb;
  pcl::fromROSMsg(*input, tmp_rgb);

  if(output_rgb==true)
  {
    for (pcl::PointCloud<pcl::PointXYZRGB>::const_iterator item = tmp_rgb.begin(); item != tmp_rgb.end(); item++)
    {
      p.x = (double)item->x;
      p.y = (double)item->y;
      p.z = (double)item->z;
      p.intensity = (float)item->rgb;//0.299f*(float)item->r + 0.587*(float)item->g + 0.114f*(float)item->b;
  ...
  double exe_time = std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count() / 1000.0;
  std::cout << "totle_exe_time:" << exe_time << std::endl;
}

static void gnss_callback(const geometry_msgs::PoseStamped::ConstPtr& input)
{
  gnss_x_temp = input->pose.position.x;
  gnss_y_temp = input->pose.position.y;
  
  if(gnss_init_is_set == false && _use_imu_gnss_init_pose == true)
  {
    gnss_init_is_set = true;
    std::cout << "gnss_init_is_set!" << std::endl;
  }
}
...
int main(int argc, char** argv)
{
  ...
  private_nh.getParam("use_imu_gnss_init_pose", _use_imu_gnss_init_pose);
  ...
  std::cout << "use_imu_gnss_init_pose: " << _use_imu_gnss_init_pose << std::endl;
  ...
  if(_use_imu_gnss_init_pose == false)
  {
  ...
  /*geo_pos_conv geo_;
  geo_.set_plane(2502.58959,12132.12869);
  geo_.set_llh_nmea_degrees(2502.60719, 12132.08992, 0.5);
  std::cout << "gggggggggg:" << geo_.x() << ",:" << geo_.y() << std::endl;*/
  ros::spin();
