...
static ros::Publisher odom_pub; //add
static nav_msgs::Odometry odom_msg; //add
...
//my define
double previous_imu_pitch_gobal;
double previous_imu_yaw_gobal;
double previous_imu_roll_gobal;
double current_imu_pitch_gobal;
double current_imu_yaw_gobal;
double current_imu_roll_gobal;
static int _use_imu_yaw = 1;
...
static void gnss_callback(const geometry_msgs::PoseStamped::ConstPtr& input)
{
  ...
  if ((_use_gnss == 1 && init_pos_set == 0) || fitness_score >= 50.0)
  {
	...
	if(_use_imu_yaw==1)
	{
		previous_pose.roll = previous_imu_pose.roll;
		previous_pose.pitch = previous_imu_pose.pitch;
		previous_pose.yaw = previous_imu_pose.yaw;
		
		current_pose.roll = current_imu_roll_gobal;
		current_pose.pitch = current_imu_pitch_gobal;
		current_pose.yaw = current_imu_yaw_gobal;
	}
  ...
  previous_imu_pose.roll = current_imu_roll_gobal;
  previous_imu_pose.pitch = current_imu_pitch_gobal;
  previous_imu_pose.yaw = current_imu_yaw_gobal;
}
...
static void imu_callback(const sensor_msgs::Imu::Ptr& input)
{
  ...
  current_imu_roll_gobal = imu_roll;
  current_imu_pitch_gobal = imu_pitch;
  current_imu_yaw_gobal = imu_yaw;
}
  ...
  if (map_loaded == 1 && init_pos_set == 1)
  {
    ...
    */
    ...
    {//add
      ...
      odom_msg.header.stamp = current_scan_time;

      odom_msg.pose.pose.position.x = ndt_pose.x;
      odom_msg.pose.pose.position.y = ndt_pose.y;
      odom_msg.pose.pose.position.z = ndt_pose.z;
      odom_msg.pose.pose.orientation.x = ndt_q.x();
      odom_msg.pose.pose.orientation.y = ndt_q.y();
      odom_msg.pose.pose.orientation.z = ndt_q.z();
      odom_msg.pose.pose.orientation.w = ndt_q.w();
...
int main(int argc, char** argv)
{
  ...
  private_nh.getParam("use_imu_yaw", _use_imu_yaw);
  ...
  std::cout << "use_imu_yaw: " << _use_imu_yaw << std::endl;
  ...
  odom_pub = nh.advertise<nav_msgs::Odometry>("/odom_imu", 10);//add odom
